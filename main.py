...
train_transforms = transforms.Compose(
        [transforms.RandomResizedCrop(size=256, scale=(0.8, 1.0)),#randomly crop to pixel 256x256
        transforms.RandomRotation(degrees=15),#rotate
        transforms.RandomHorizontalFlip(),
        transforms.CenterCrop(size=224),  #224x224
        transforms.ToTensor(),     #transform range to be [0,1]
        transforms.Normalize([0.485, 0.456, 0.406],#mean values of red,green,blue channel
                             [0.229, 0.224, 0.225]) #sd of red,green,blue channel
])
...
def train_and_valid(model, loss_function, optimizer, epochs=25):
    ...
    for epoch in range(epochs):
        ...
        torch.save(model.state_dict(), 'trained_models/my-10_model_' + str(epoch + 1) + '.pth')
    return model, record
